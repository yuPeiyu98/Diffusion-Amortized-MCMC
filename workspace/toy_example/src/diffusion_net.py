...
class Diffusion_Unet(nn.Module):
    ...
    def forward(self, z, logsnr, xemb):
        ...
        logsnr_input = (
            torch.arctan(torch.exp(-0.5 * torch.clamp(logsnr, min=-20., max=20.))) / (0.5 * np.pi)
        )
