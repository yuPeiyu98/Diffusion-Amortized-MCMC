...
def sample_langevin_post_z_with_prior_p(z, x, netG, netE, netF, g_l_steps, g_llhd_sigma, g_l_with_noise, g_l_step_size, verbose = False):
    ...
    for i in range(g_l_steps):
        x_hat = netG(z)
        g_log_lkhd = 1.0 / (2.0 * g_llhd_sigma * g_llhd_sigma) * torch.mean((x_hat - x) ** 2, dim=[1,2,3]).sum()
...
def sample_langevin_post_z_with_prior_nce(z, x, netG, netE, g_l_steps, g_llhd_sigma, g_l_with_noise, g_l_step_size, verbose = False):
    mystr = "Step/cross_entropy/recons_loss: "
...
def gen_samples_with_diffusion_prior_E(b, device, netQ, netG, netE):
    ...
    with torch.no_grad():
        ...
        if isinstance(netE, list):
            logit = netE[1](zk_prior) + netE[2](zk_prior)
        else:
