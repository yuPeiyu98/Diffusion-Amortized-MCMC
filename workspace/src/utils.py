...
def stitch_images(inputs, *outputs, img_per_row=2):
    ...
    for ix in range(len(inputs)):
        xoffset = int(ix % img_per_row) * width * columns \
                + int(ix % img_per_row) * gap
        yoffset = int(ix / img_per_row) * height

        for cat in range(len(images)):
            im = np.array(
                (images[cat][ix]).cpu()).astype(
                                         np.uint8).squeeze()
...
def imsave(img, path):
    im = Image.fromarray(
        img.cpu().numpy().astype(np.uint8).squeeze())
    im.save(path)
...
DEFAULT_CONFIG = {    
    ...
    'MAX_T': 6,                     # number of noise levels
